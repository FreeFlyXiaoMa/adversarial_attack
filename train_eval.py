...
class SelfProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        data = pd.read_csv('./data/trian.csv')
        # random.seed(0)
        # random.shuffle(data)
...
def main():
    ...
    if arg_dic['do_predict']:
        ...
        output_predict_file = os.path.join(arg_dic['output_dir'], "predict_results.csv")
        with tf.gfile.GFile(output_predict_file, "w") as writer:

            tf.logging.info("***** Predict results *****")
            for sam, prediction in zip(predict_examples, result):
                # print('sam::',sam,'prediction::',prediction)
