...
def evaluate(args, model, tokenizer, ngram_dict, processor, label_list):
    eval_dataset = load_examples(args, tokenizer, ngram_dict, processor, label_list, mode="dev")
...
def predict(args, model, tokenizer, ngram_dict, processor, label_list):
    test_dataset = load_examples(args, tokenizer, ngram_dict, processor, label_list, mode="test")
    # Run prediction for full data
    if args.local_rank == -1:
        eval_sampler = SequentialSampler(test_dataset)
    else:
        eval_sampler = DistributedSampler(test_dataset)  # Note that this sampler samples randomly
    ...
    for batch in tqdm(eval_dataloader, desc="Predict"):
        batch = tuple(t.to(args.device) for t in batch)
        input_ids, input_mask, segment_ids, label_ids, input_ngram_ids, ngram_position_matrix, \
        ngram_lengths, ngram_seg_ids, ngram_masks = batch
    ...
    preds = np.argmax(preds[0], axis=1)
    # return compute_metrics(args.task_name, preds, out_label_ids)
    pickle.dump(preds,open('lcqmc_pred.pkl','wb'))
...
def main():
    ...
    parser.add_argument("--data_dir",
                        default='/E/home/mayajun/PycharmProjects/adversarial_attack/baseline2/datasets',
                        type=str,

                        help="The input data dir. Should contain the .tsv files (or other data files) for the task.")
    parser.add_argument("--bert_model", default='/E/home/mayajun/PycharmProjects/adversarial_attack/ZEN_ft_SPM_v0.1.0', type=str,
    ...
    parser.add_argument("--output_dir",
                        default='/E/home/mayajun/PycharmProjects/adversarial_attack/baseline2/output',
                        type=str,
