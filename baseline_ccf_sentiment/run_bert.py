...
def read_examples(input_file, is_training):
    ...
    if is_training:
        for val in df[['id','content','title','label']].values:
            examples.append(InputExample(guid=val[0],text_a=str(val[1]),text_b=str(val[2]),label=val[3]))
    else:
        for val in df[['id','content','title']].values:
            examples.append(InputExample(guid=val[0],text_a=str(val[1]),text_b=str(val[2]),label=None))
